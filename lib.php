...
class repository_planetestream extends repository
{
	...
	global $SESSION;
        ...
        $this->feed_url = $this->get_url() . '/VLE/Moodle/Default.aspx?search=' . urlencode($keyword) . '&format=5&pageindex=' . $pageindex . '&orderby=' . $sort . '&cat=' . $cat . '&show=' . $show . '&delta=' . $this->funcObfuscate($USER->username) . '&checksum=' . $this->funcGetChecksum();
	//echo($this->feed_url);
